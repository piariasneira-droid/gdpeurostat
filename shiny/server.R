...
server <- function(input, output, session) {
  ...
  # Reactives: parquet queries ----
  
  # All NUTS2 regions for the selected year and variables ----
  df_plot <- reactive({
  ...
  output$data_table <- renderDT({
    ...
    # Reverse COUNTRIES lookup for display ----
    ...
    datatable(
      ...
      formatStyle(
        columns    = y_label,
        background = styleColorBar(df_tbl[[y_label]], "#526DB033"),
